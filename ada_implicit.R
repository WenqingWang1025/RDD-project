...
plot_data <- ada_state %>%
  ...
  left_join(
    ada_pop %>% select(State_Original, State_Name, State_Abbr, Avg_Population, Weight),
...
ggsave("ada_plot1_implicit_vs_population.png", p1, width = 10, height = 7, dpi = 300)
...
p2 <- ggplot(plot_data, aes(x = seats_1990, y = implicit_weight_pct)) +
  ...
  geom_text_repel(aes(label = state_abbr), size = 3, fontface = "bold",
                  max.overlaps = 50, box.padding = 0.5, point.padding = 0.3) +
  scale_color_gradient(low = "#6495ED", high = "#00008B", 
                       name = "Population\n(millions)") +
  labs(
    title = "Implicit RD Weights vs. Congressional Seats",
    subtitle = sprintf("ADA Analysis | Correlation: %.3f", cor_seats),
...
ggsave("ada_plot2_implicit_vs_seats.png", p2, width = 10, height = 7, dpi = 300)
...
p3 <- ggplot(plot_data, aes(x = population_weight_pct, y = implicit_weight_pct)) +
  ...
  labs(
    ...
    subtitle = sprintf("ADA Analysis | Correlation: %.3f | Gray line = perfect alignment", 
                       cor_theory),
...
ggsave("ada_plot3_implicit_vs_theory.png", p3, width = 10, height = 7, dpi = 300)
