...
data_clean <- data %>% filter(!is.na(lagdemvoteshare) & !is.na(democrat))
years <- sort(unique(data_clean$year))

cat("========================================\n")
cat("Democrat by Year Analysis\n")
cat("========================================\n")
...
for (yr in years) {
  ...
  if (n_left >= 15 & n_right >= 15) {
    tryCatch({
      ...
      mu_left <- mean(data_left$democrat, na.rm = TRUE)
      mu_right <- mean(data_right$democrat, na.rm = TRUE)
...
write.csv(results_df, 
          file = file.path(output_dir, "democrat_by_year_detailed_results.csv"),
          row.names = FALSE)
...
write.csv(simple_df,
          file = file.path(output_dir, "democrat_by_year_simple_results.csv"),
          row.names = FALSE)
...
write.csv(weight_df,
          file = file.path(output_dir, "democrat_by_year_weights.csv"),
          row.names = FALSE)
cat("  3. Weights saved\n")
...
write.csv(summary_stats,
          file = file.path(output_dir, "democrat_by_year_summary_statistics.csv"),
          row.names = FALSE)
...
metadata <- data.frame(
  ...
  Value = c(
    "democrat (Probability Democrat Runs)",
    "lagdemvoteshare - 0.5",
    "0",
    "1 (Local Linear)",
    "Triangular",
...
write.csv(metadata,
          file = file.path(output_dir, "democrat_by_year_metadata.csv"),
          row.names = FALSE)
...
png(file.path(output_dir, "dem_forest_plot.png"), 
...
plot(results_sorted$TE, y_positions,
     ...
     xlab = "Treatment Effect (Probability Democrat Runs)", ylab = "",
     main = "Treatment Effects on Probability Democrat Runs by Year",
...
png(file.path(output_dir, "dem_time_series.png"), 
...
plot(results_sorted$year, results_sorted$TE,
     ...
     xlab = "Year", ylab = "Treatment Effect (Probability Democrat Runs)",
     main = "Treatment Effects on Probability Democrat Runs Over Time",
